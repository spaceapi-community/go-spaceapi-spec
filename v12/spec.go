...
type Root struct {
	...
	Feeds []*FeedsItems `json:"feeds,omitempty"`
	...
	Sensors []*SensorsItems `json:"sensors,omitempty"`
...
// SensorsItems
type SensorsItems struct {

	// A mapping of measuring locations to temperature values. The values should match the basic regular expression <code>^[+-]?[0-9]+(\.[0-9]+)?[FCK]$</code>. Example: <samp>{'kitchen':'48F', 'storage':'-273.1K'}</samp>
...
func (strct *FeedsItems) MarshalJSON() ([]byte, error) {
	buf := bytes.NewBuffer(make([]byte, 0))
	buf.WriteString("{")
	comma := false
	// "Name" field is required
...
func (strct *FeedsItems) UnmarshalJSON(b []byte) error {
	nameReceived := false
	urlReceived := false
	var jsonMap map[string]json.RawMessage
	if err := json.Unmarshal(b, &jsonMap); err != nil {
...
func (strct *Root) MarshalJSON() ([]byte, error) {
	...
	if strct.Icon == nil {
		return nil, errors.New("icon is a required field")
	}
...
func (strct *Root) UnmarshalJSON(b []byte) error {
	...
	// check if icon (a required property) was received
	if !iconReceived {
		return errors.New("\"icon\" is required but was not present")
	}
